SELECT e.id                                                                                                                                                                                                                                                            AS event_id,
       ...
       --Actual Count Comps
       CAST(COALESCE(SUM(oi.quantity - oi.refunded_quantity) FILTER (WHERE o.on_behalf_of_user_id IS NOT NULL AND h.hold_type = 'Comp'), 0) AS BIGINT)                                                                                                                 AS box_office_actual_comp_count,
       CAST(COALESCE(SUM(oi.quantity - oi.refunded_quantity) FILTER (WHERE o.on_behalf_of_user_id IS NULL AND h.hold_type = 'Comp'), 0) AS BIGINT)                                                                                                                     AS online_actual_comp_count,
